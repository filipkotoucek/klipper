...
class HD44780:
    ...
    def send(self, cmd_type, data):
        self.mcu.send(cmd_type.encode(self.oid, data),
                      reqclock=BACKGROUND_PRIORITY_CLOCK, cq=self.cmd_queue)
        #logging.debug("hd44780 %d %s", cmd_type==self.send_cmds_cmd, repr(data))
    def flush(self):
    ...
    def init(self):
        ...
        for i, cmds in enumerate(init):
            ...
            cmd = self.send_cmds_cmd.encode(self.oid, cmds)
...
class ST7920:
    ...
    def send(self, cmd_type, data):
        self.mcu.send(cmd_type.encode(self.oid, data),
                      reqclock=BACKGROUND_PRIORITY_CLOCK, cq=self.cmd_queue)
        #logging.debug("st7920 %d %s", cmd_type==self.send_cmds_cmd, repr(data))
    def flush(self):
        # Find all differences in the framebuffers and send them to the chip
        for new_data, old_data, fb_id in self.framebuffers:
            if new_data == old_data:
